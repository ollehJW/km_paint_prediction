...
class ConstructTotalDataset(Dataset):
    """
    Construct pytorch Dataset from file list.
    ...
    def __init__(self, image_data, table_data, target_list, phase = 'train'):
    ...
    def __getitem__(self, index):
        ...
        target = self.target_list[index]
        table_sample = self.table_data[index, :]
        image = cv2.imread(file_name)
        image = transform_dict[self.phase](image)
        return {'image': image, 'table': table_sample, 'target': target}
...
class TotalDatasetGenerator(object):
    """
    Construct pytorch DataLoader from file list.
    ...
    pytorch.DataLoader
    """
    def __init__(self, file_list, table_data, target_list, batch_size = 16, phase = 'train'):
        self.file_list = file_list
        self.table_data = table_data
        self.target_list = target_list
        self.batch_size = batch_size
    ...
    def dataloader(self):
        ...
        if self.phase == 'train':
            train_dataset = ConstructTotalDataset(self.file_list, self.table_data, self.target_list, phase = 'train')
            return dict({'train': DataLoader(train_dataset, batch_size=self.batch_size, shuffle=True)})
        
        else:
            test_dataset = ConstructTotalDataset(self.file_list, self.table_data, self.target_list, phase = self.phase)
            return dict({'test': DataLoader(test_dataset, batch_size=self.batch_size, shuffle=False)})